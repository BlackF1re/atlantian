...
LOOP=$(losetup --find --show --partscan "$OUT"); BOOT=; ROOT=
cleanup(){ set +e; [ -n "${ROOT:-}" ] && mountpoint -q "$ROOT" && umount "$ROOT"; [ -n "${BOOT:-}" ] && mountpoint -q "$BOOT" && umount "$BOOT"; [ -n "${ROOT:-}" ] && rmdir "$ROOT"; [ -n "${BOOT:-}" ] && rmdir "$BOOT"; losetup -d "$LOOP"; }
trap cleanup EXIT
...
rsync -aHAX --numeric-ids "$ROOTFS/" "$ROOT/"
