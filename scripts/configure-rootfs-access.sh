...
# AtlANTian is root-only.  Remove an account only if it exists; this keeps the
# script repeatable and does not make a build depend on an old rootfs state.
if chroot "$ROOT" id -u paul >/dev/null 2>&1; then
  chroot "$ROOT" userdel -r paul || true
fi
rm -rf "$ROOT/home/paul" "$ROOT/etc/sudoers.d/atlantian-deploy"

# An empty hash is intentional for the first boot and is allowed by sshd below.
# `passwd` later replaces it with a normal hash, which also suppresses the
# login warning installed at the end of this file.
chroot "$ROOT" passwd -d root
chroot "$ROOT" passwd -u root || true

# The build host key is optional.  It grants only key-based root access; no
# secret material is ever put in the image.
